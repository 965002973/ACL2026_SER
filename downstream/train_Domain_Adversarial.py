...
from data_DANN import load_DANN_features, train_valid_test_dataloader
from model import EmotionDANN
from utils_DANN import train_one_epoch, validate_and_test
import logging
...
def train_DANN(cfg: DictConfig):
    ...
    if cfg.model.adversarial_domain == 'syn_dict':
        domain_label_dict = syn_dict 
    elif cfg.model.adversarial_domain == 'vocoder_dict':
        domain_label_dict = vocoder_dict 
    elif cfg.model.adversarial_domain == 'model_dict':
        domain_label_dict = model_dict 
    else:
        logger.info("adversarial_domain NOT EXIST")
    ...
    for epoch in range(total_epochs):
        ...
        train_loss, train_emo_loss, train_domain_loss = train_one_epoch(model, optimizer, criterion, train_loader, epoch, total_epochs, max_alpha, device)
        scheduler.step()
        ...
        logger.info(
            ...
            f"emo loss {train_emo_loss/len(train_loader):.4f} | "
            f"domain loss {train_domain_loss/len(train_loader):.4f} | "
