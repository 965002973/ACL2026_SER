...
def train_one_epoch(
    ...
    optimizer,
    criterion,
    real_loader,          # 真实域 DataLoader（每步吐出 list[dict]）
    syn_loader,           # 合成域 DataLoader（每步吐出 list[dict]）
    collator,             # SpeechDataset.collator（用于合并后统一 padding）
    epoch,
    total_epochs,
    max_alpha,
    device,
    ...
    # alpha 预热 / schedule 保持你的实现
    p = epoch / total_epochs
    alpha = (2.0 / (1.0 + np.exp(-10 * p)) - 1.0) * max_alpha

    syn_iter = cycle(syn_loader)

    # 以真实域 loader 的长度定义一个 epoch（大周期由真实数据定义）
    for real_samples in real_loader:
        syn_samples = next(syn_iter)

        # real_samples / syn_samples 必须是 list[dict]
        merged_samples = real_samples + syn_samples

        # 合并后统一 collate（一次 padding）
        batch = collator(merged_samples)
        ...
        loss_dom = criterion(domain_pred, domain_labels.long())

        loss = 1.0 * loss_emo + 1.0 * loss_dom

        train_loss += float(loss.item())
        train_emo_loss += loss_emo.item()
        train_domain_loss += loss_dom.item()
